...
bool WaitWithPlatformEvents(std::unique_lock<std::mutex>& lock, Predicate&& ready,
    ResourceClock::time_point deadline = ResourceClock::time_point::max()) {
    if (ResourceWaitRequiresEventLoop()) {
        // 等待 worker 时释放资源锁并让出事件循环，使代理的 File 异步读取能够完成
        while (!ready() && ResourceClock::now() < deadline) {
            lock.unlock();
            YieldResourceWaitToEventLoop();
            lock.lock();
        }
        return true;
    }
...
struct ExecutionState {
    static constexpr std::size_t noAdmission = std::numeric_limits<std::size_t>::max();
    struct Admission {
        ...
        std::size_t nextFree{noAdmission};
    };
    ...
    explicit ExecutionState(const ResolvedResourceConfiguration& config)
        ...
        for (std::size_t i = heavyIndex; i != 0u;) {
            --i;
            admissions[i].nextFree = firstFreeAdmission;
            firstFreeAdmission = i;
        }
    ...
    void RequestMemorySampleLocked(ResourceClock::time_point boundary) noexcept {
        if (configuration.mode != CodecResourceMode::Adaptive) { return; }
        memorySampleAfter = boundary;
        memoryControlRequested = true;
        controllerChanged.notify_all();
    }
    void MemoryPreparationCompletedLocked(ResourceClock::time_point now) noexcept {
        controller.memory.lastPreparationAt = now;
        controller.progressAt = now;
        if (controller.grant.used && !controller.grant.sequence) { controller.grant.completedAt = now; }
        RequestMemorySampleLocked(now);
    }
    void ReservedLocked(std::uint64_t bytes, std::optional<std::uint64_t> sequence = {}) noexcept {
        ...
        if (controller.phase == ResourceControlPhase::Recover) {
            debug.gateOpen = false;
            memoryControlRequested = true;
            controllerChanged.notify_all();
        }
        ClearDemandLocked();
    ...
    void Event(ResourceEventKind kind, std::uint64_t sequence = 0u,
        ...
        if (kind == ResourceEventKind::MemoryControl || kind == ResourceEventKind::Limits ||
            ...
            event.memoryPreviewRatio = controller.memory.previewRatio;
    ...
    void ReleaseLocked(std::size_t index, std::uint64_t generation) noexcept {
        ...
        if (admission.committed || admission.completion == BlockCompletion::Succeeded) {
            controller.progressAt = ResourceClock::now();
            controller.memory.waitSince.reset();
            // 单块退休立即观测，避免把周期采样间隔变成逐块节流
            if (controller.phase != ResourceControlPhase::Normal) {
                RequestMemorySampleLocked(*controller.progressAt);
            }
        }
        ...
        if (index == heavyIndex) {
            ...
            admission.nextFree = firstFreeAdmission;
    ...
    // driver 的容量和完成事件与 worker 的任务唤醒分开，避免每次归还缓冲都唤醒整个线程池
    ...
    std::size_t firstFreeAdmission{noAdmission};
    ...
    std::optional<ResourceClock::time_point> memorySampleAfter;
...
struct DataCodecExecutionResources::Impl {
    static std::size_t ScratchWorkerHint(const ResolvedResourceConfiguration& config) noexcept {
        // 硬件并发度只用于预留空闲缓冲的索引容量，Unlimited 线程仍按任务需求增长
        return config.threaded && config.threadMode == CodecThreadMode::Unlimited
            ? std::max<std::size_t>(1u, config.initialSample.allowedComputeThreads.value_or(1u))
            : config.computeCeiling;
    }

    explicit Impl(const ResolvedResourceConfiguration& config)
        : state(std::make_shared<ExecutionState>(config)),
          scratch(2u * (ScratchWorkerHint(config) + std::min<std::size_t>(ScratchWorkerHint(config), 8u))),
...
bool DataCodecExecutionResources::BeginRun() noexcept {
    try {
        ...
        m_impl->scratch.SetRetainedCount(state.debug.limits.ownedStorageLimitBytes != 0u &&
            ...
            ? 2u * (state.UnlimitedThreads() ? std::max(state.admissions.size(), Impl::ScratchWorkerHint(state.configuration) + 1u) :
...
bool DataCodecExecutionResources::EndRun() noexcept {
    ...
    // 正常请求保留 worker 和压缩器，取消后的线程在这里收束
    if (!failed && state.UnlimitedThreads() && stopped) {
        {
            std::lock_guard lock(state.mutex);
    ...
    if (!failed) { m_impl->scratch.ClearFixed(); }
    state.changed.notify_all();
    return !failed;
}
...
std::optional<SlotLease> DataCodecExecutionResources::TryAcquireSlot(
    ...
    if (state->firstFreeAdmission == ExecutionState::noAdmission && state->UnlimitedThreads()) {
        // 扩展前先检查本次字节需求，重型阶段保留稳定索引
        if (!state->capacity->Snapshot().CanReserve(bytes)) {
            state->DemandLocked(bytes, MemoryDemandKind::Block);
            return std::nullopt;
        }
        const auto oldSize = state->admissions.size();
        const auto maximum = std::min(state->admissions.max_size(), state->queue.max_size());
        if (oldSize == maximum) {
            throw std::length_error("admission storage exhausted");
        }
        // 槽位和环形队列几何增长，空闲索引链避免反复扫描活动记录
        const auto nextSize = oldSize + std::min(oldSize, maximum - oldSize);
        std::vector<std::shared_ptr<TerminalWork>> grown(nextSize);
        state->admissions.resize(nextSize);
        ...
        state->queueHead = 0u;
        for (std::size_t i = nextSize; i != oldSize;) {
            --i;
            state->admissions[i].nextFree = state->firstFreeAdmission;
            state->firstFreeAdmission = i;
        }
    }
    if (const auto i = state->firstFreeAdmission; i != ExecutionState::noAdmission) {
        ...
        state->firstFreeAdmission = admission.nextFree;
...
void DataCodecExecutionResources::CompleteMemoryPreparation() noexcept {
    ...
    std::lock_guard lock(state.mutex);
    const auto now = ResourceClock::now();
    state.MemoryPreparationCompletedLocked(now);
    state.Event(ResourceEventKind::Complete);
    state.changed.notify_all();
}

bool DataCodecExecutionResources::SynchronizeMemoryAfterPreparation() {
    auto& state = *m_impl->state;
    std::unique_lock lock(state.mutex);
    if (state.configuration.mode != CodecResourceMode::Adaptive) { return !state.debug.runStopped; }
    if (!state.debug.runActive || state.driver != std::this_thread::get_id() || state.IsWorkerLocked()) {
        throw std::logic_error("memory preparation synchronization requires the active driver");
    }
    const auto boundary = ResourceClock::now();
    const auto request = state.debug.requestId;
    state.MemoryPreparationCompletedLocked(boundary);
    const auto observed = [&] {
        return state.debug.closing || state.debug.runStopped || !state.debug.runActive ||
            state.debug.requestId != request || state.debug.resourceSample.sampledAt > boundary;
    };
    if (!WaitWithPlatformEvents(lock, observed)) { state.changed.wait(lock, observed); }
    return state.debug.runActive && !state.debug.runStopped && !state.debug.closing && state.debug.requestId == request;
}
...
bool DataCodecExecutionResources::Submit(const SlotLease& slot, const std::shared_ptr<TerminalWork>& work) noexcept {
    ...
    if (!inlineWork) { state.workAvailable.notify_one(); }
    if (inlineWork) {
        bool success = false;
        ...
        bool wakeWorker = false;
        {
            ...
            while (!state->debug.closing && !state->retireWorkers && !state->CanStartLocked()) {
                if (state->AutomaticCpu() && state->debug.queuedTasks != 0u && !state->debug.runStopped) {
                    state->workAvailable.wait_until(lock, state->cpuBudget.NextWake(ResourceClock::now(), state->debug.activeComputeUnits));
                } else { state->workAvailable.wait(lock); }
            }
            ...
            wakeWorker = state->CanStartLocked();
        }
        // 可执行任务逐个传递唤醒，额度增加时也能填满并行窗口
        if (wakeWorker) { state->workAvailable.notify_one(); }
        bool success = false;
        std::optional<CodecFailureRecord> failure;
...
std::optional<CodecFailureRecord> DataCodecExecutionResources::PublishLimitsLocked(
    ...
    m_impl->scratch.SetRetainedCount(retain ? 2u * (state.UnlimitedThreads() ?
        std::max(state.admissions.size(), Impl::ScratchWorkerHint(state.configuration) + 1u) :
        std::max(limits.slotLimit, limits.computeLimit)) : 0u,
        trim || limits.ownedStorageLimitBytes < before.ownedStorageLimitBytes);
...
std::optional<CodecFailureRecord> DataCodecExecutionResources::ApplyControlLocked(
    ...
    if (previousPhase != state.controller.phase) {
...
void DataCodecExecutionResources::ControlMain() noexcept {
    ...
    for (;;) {
        ...
        const bool preparationSample = state.memorySampleAfter && sample.sampledAt <= *state.memorySampleAfter;
        const bool memoryDue = memoryAdaptive && (now >= nextSampleAt || pressureNotified || preparationSample);
        if (memoryDue || sampleCpu) {
            const bool sampleMemory = memoryDue;
            lock.unlock();
            if (sampleMemory) {
        ...
        if (state.memorySampleAfter && sample.sampledAt > *state.memorySampleAfter) { state.memorySampleAfter.reset(); }
        ...
        m_impl->scratch.TrimRetained();
        if (failure) { RecordFailure(*failure); }
        if (observed != before || failure || memoryDue) {
            state.workAvailable.notify_one();
            state.changed.notify_all();
        }
        lock.lock();
...
void DataCodecExecutionResources::CancelAndWaitRun() noexcept {
    ...
    std::unique_lock lock(state.mutex);
    const auto drained = [&] { return state.runningTasks == 0u; };
    if (!WaitWithPlatformEvents(lock, drained)) { state.changed.wait(lock, drained); }
    lock.unlock();
    SetWaitReason(ResourceWaitReason::None);
}
...
void DataCodecExecutionResources::WaitForChange(std::uint64_t observedEpoch, std::stop_token stop) {
    ...
    const auto deadline = state.debug.byteWaiting
        ? ResourceClock::now() + resource_control::sampleInterval : ResourceClock::time_point::max();
    if (WaitWithPlatformEvents(lock, changed, deadline)) { return; }
...
std::size_t DataCodecExecutionResources::WorkerCapacity() const noexcept {
    const auto& configuration = m_impl->state->configuration;
    return configuration.threaded ? configuration.computeCeiling : 1u;
}
