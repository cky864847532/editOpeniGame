...
LeafPackage EncodeFixture(const TestDataset& dataset, bool outerCompressed, bool independentAttributes = false) {
    ...
    auto encoded = Encode(EncodeRequest{.input = EncodeInput::LeafAdapter(&adapter),
...
class DiscardTopologyObserver final : public IDecodeTopologyBlockObserver {
public:
    bool BeginConnectivityTopology(const ConnectivityTopologyDecodeInfo&, std::string*) override { return true; }
    bool ObserveConnectivityBlock(DecodedConnectivityTopologyBlock, std::string*) override { return true; }
    bool EndConnectivityTopology(std::string*) override { return true; }
};
...
std::uint64_t Run(const DecodeStorageAnalysisRequest& analysis, bool framed, bool reduced = false, bool threaded = false) {
    ...
    CodecRunScope scope(root);
    std::unique_ptr<IDecodeAdapter> adapter = analysis.adapterBackedAttributes
        ? std::unique_ptr<IDecodeAdapter>(std::make_unique<iGame::iGameDecodeAdapter>())
        : std::unique_ptr<IDecodeAdapter>(std::make_unique<TestDecodeAdapter>());
    Assembly testAssembly;
    iGame::iGameFramePackageDecodeAssembly hostAssembly;
    IFramePackageDecodeAssembly* assembly = analysis.adapterBackedAttributes
        ? static_cast<IFramePackageDecodeAssembly*>(&hostAssembly) : &testAssembly;
    DecodeSession session;
    DecodePackageRequest request{.inputReader = analysis.inputReader, .leafAdapter = adapter.get(),
        .frameAssembly = framed ? assembly : nullptr, .attributeSelection = analysis.attributeSelection,
        .attributeTargets = analysis.attributeTargets};
    request.configuration.execution.topologyOutputMode = analysis.topologyOutputMode;
    if (analysis.topologyOutputMode == TopologyDecodeOutputMode::ObserverOnly) {
        request.configuration.execution.topologyBlockObserver = std::make_shared<DiscardTopologyObserver>();
    }
    ...
    if (reduced && floor) {
        ...
        if (!framed && !analysis.adapterBackedAttributes) {
            Ensure(static_cast<TestDecodeAdapter&>(*adapter).Committed() &&
                !static_cast<TestDecodeAdapter&>(*adapter).Points().empty(), "real output was not produced");
        }
    }
    std::cout << "storage_case framed=" << framed << " host=" << analysis.adapterBackedAttributes
        << " selection=" << static_cast<int>(analysis.attributeSelection) << " reduced=" << reduced
        << " threaded=" << threaded << " observer=" << (analysis.topologyOutputMode == TopologyDecodeOutputMode::ObserverOnly)
...
void Supplement(const LeafPackage& leaf) {
    ...
    const auto full = AnalyzeDecodeStorage({.inputReader = input});
    ...
    const auto decoded = DecodePackageInRun({.inputReader = input, .leafAdapter = &adapter,
        .attributeSelection = AttributeSelectionMode::Explicit, .attributeTargets = {{0u, {}, 0u}}}, root, &session);
    Ensure(decoded.success, decoded.failure ? FormatCodecFailure(*decoded.failure) : "initial partial decode failed");
    DecodeStorageAnalysisRequest request{.inputReader = input, .attributeSelection = AttributeSelectionMode::AllAvailable};
...
void Sequence(const LeafPackage& leaf, bool host, const LeafPackage* keyLeaf = nullptr) {
    ...
    for (const auto& source : {reference, input}) {
        ...
        Assembly testAssembly;
        iGame::iGameFramePackageDecodeAssembly hostAssembly;
        IFramePackageDecodeAssembly* assembly = host ?
            static_cast<IFramePackageDecodeAssembly*>(&hostAssembly) : &testAssembly;
        const auto decoded = DecodePackageInRun({.inputReader = source, .frameAssembly = assembly}, root, &session);
    ...
    const auto missing = AnalyzeDecodeStorage({.inputReader = input});
...
LeafPackage ReferenceFixture(const LeafPackage& original, NumericArrayReferenceKind kind, std::size_t targetIndex) {
    auto leaf = original;
    TestDecodeAdapter adapter;
    const auto decoded = DecodePackage({.inputReader = LeafReader(leaf), .leafAdapter = &adapter});
...
void LazyTemporal(const LeafPackage& keyLeaf, const LeafPackage& temporalLeaf) {
    ...
    auto required = root.Caches().ReferenceCache()->RequireFrame({identities->at(0u), 0u});
    {
        ...
        Assembly assembly;
        auto decoded = DecodePackageInRun({.inputReader = reference, .frameAssembly = &assembly,
            .attributeSelection = AttributeSelectionMode::None}, root, &keySession);
        Ensure(decoded.success, decoded.failure ? FormatCodecFailure(*decoded.failure) : "lazy key decode failed");
    }
    DecodeSession session;
    session.ConfigureReferences(root, identities);
    Assembly assembly;
    auto initial = DecodePackageInRun({.inputReader = input, .frameAssembly = &assembly,
        .attributeSelection = AttributeSelectionMode::None}, root, &session);
    ...
    DecodeStorageAnalysisRequest request{.inputReader = input, .referenceReaders = {reference}};
...
void MetadataOnly(const LeafPackage& leaf) {
    ...
    const auto result = AnalyzeDecodeStorage({.inputReader = reader});
...
void UnlimitedResources() {
    ...
    for (const auto& dataset : {MakeAdapterRoundTripDataset(), MakePipelineContractUnstructuredDataset()}) {
        ...
        for (const auto mode : {CodecResourceMode::Fixed, CodecResourceMode::Unlimited}) {
            ...
            auto encoded = Encode({.input = EncodeInput::LeafAdapter(&encodeAdapter),
            ...
            const auto decoded = DecodePackage({.inputReader = reader, .leafAdapter = &adapter, .resources = resources});
            ...
            if (mode == CodecResourceMode::Unlimited) {
                ...
                const auto fixedFailure = DecodePackage({.inputReader = reader, .leafAdapter = &denied,
                    .resources = {.mode = CodecResourceMode::Fixed, .maxComputeThreads = 2u, .ownedStorageLimitBytes = 0u}});
                ...
                const auto bridge = iGame::DecodeiGameWasmDataCodecMemory(bytes, *bytes, false,
                    iGame::iGameWasmTopologyOutputMode::CommitToAdapter, {}, unlimited);
...
void MultiBatchPolyhedronAdmission() {
    ...
    iGame::iGameEncodeAdapter adapter(mesh);
    auto encoded = Encode({.input = EncodeInput::LeafAdapter(&adapter),
        .output = EncodeOutput::Memory(EncodePackageKind::LeafPackage),
...
int RunDataCodecStorageAnalysisTests() try {
    ...
    for (const bool topology : {false, true}) {
        ...
        for (const bool compressed : {false, true}) {
            ...
            const auto cancellation = AnalyzeDecodeStorage({.inputReader = input, .stopToken = cancelled.get_token()});
    ...
    for (const bool polyhedron : {false, true}) {
        ...
        iGame::iGameEncodeAdapter adapter(source);
        auto encoded = Encode({.input = EncodeInput::LeafAdapter(&adapter),
            .output = EncodeOutput::Memory(EncodePackageKind::LeafPackage)});
    ...
    const auto badTarget = AnalyzeDecodeStorage({.inputReader = LeafReader(keyLeaf),
